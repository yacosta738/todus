<template>
  <div>
    <h2 id="page-heading" data-cy="CustomerHeading">
      <span v-text="$t('todusApp.customer.home.title')" id="customer-heading">Customers</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('todusApp.customer.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'CustomerCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('todusApp.customer.home.createLabel')"> Create a new Customer </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && customers && customers.length === 0">
      <span v-text="$t('todusApp.customer.home.notFound')">No customers found</span>
    </div>
          ...
          <tr v-for="customer in customers" :key="customer.id" data-cy="entityTable">
            ...
            <td>
              <span v-for="(follower, i) in customer.followers" :key="follower.id"
                >{{ i > 0 ? ', ' : '' }}
                <router-link class="form-control-static" :to="{ name: 'CustomerView', params: { customerId: follower.id } }">{{
                  follower.id
                }}</router-link>
              </span>
            </td>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="todusApp.customer.delete.question" data-cy="customerDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="todus-delete-customer-heading" v-text="$t('todusApp.customer.delete.question', { id: removeId })">
      ...
      <div slot="modal-footer">
        ...
        <button
          ...
          id="todus-confirm-delete-customer"
