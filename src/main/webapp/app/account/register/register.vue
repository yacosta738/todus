<template>
  <div class="flex flex-col justify-center items-center px-8 pt-52">
    <!--Alerts-->
    <alerts v-if="success" :show="success" strong-and-text :text="$t('register.messages.success')"
            :alert-type="typeSuccess"/>
    <alerts v-if="error" :show="error" strong-and-text :text="$t('register.messages.error.fail')"
            :alert-type="typeError"/>
    <alerts v-if="errorUserExists" :show="errorUserExists" strong-and-text
            :text="$t('register.messages.error.userexists')" :alert-type="typeError"/>
    <alerts v-if="errorEmailExists" :show="errorEmailExists" strong-and-text
            :text="$t('register.messages.error.emailexists')" :alert-type="typeError"/>

    <form class="w-1/3" id="register-form" name="registerForm" role="form" v-on:submit.prevent="register()"
          v-if="!success" no-validate>
      <div
        class="logo twitter-color w-24 transition-3 twitter-light-blue-on-hover p-2 ml-2 cursor-pointer rounded-full">
      ...
      <h1 v-text="$t('register.title')" id="register-title" data-cy="registerTitle"
          class="text-6xl text-black mb-8">Registration</h1>
      <div class="mb-8 w-full">
        <label class="block">
          <input v-model="$v.registerAccount.login.$model"
                 id="username" name="login" required minlength="1" maxlength="50"
        ...
        <div class="mb-8 w-full">
          <label class="block">
            <input
              type="password" id="firstPassword" name="password"
              ...
              v-bind:placeholder="$t('global.form[\'newpassword.placeholder\']')" data-cy="firstPassword"
              class="form-input mt-1 block p-4 w-full border rounded border-gray-400 focus-within:border-blue-100">
          </label>
        ...
        <div class="mb-8 w-full">
          <label class="block">
            <input
              ...
              v-bind:placeholder="$t('global.form[\'confirmpassword.placeholder\']')" data-cy="secondPassword"
              class="form-input mt-1 block p-4 w-full border rounded border-gray-400 focus-within:border-blue-100">
          </label>
          <div v-if="$v.confirmPassword.$dirty && $v.confirmPassword.$invalid">
            ...
            <small class="form-text-error" v-if="!$v.confirmPassword.sameAsPassword"
                   v-text="$t('global.messages.error.dontmatch')">
              The password and its confirmation do not match!
            </small>
          </div>
        </div>
        <p>CUSTOMER COMPONENT HERE</p>
      <button class="text-white bg-blue-500 w-full rounded-full text-2xl text-extrabold p-4 mb-8" type="submit"
              :disabled="$v.$invalid" v-text="$t('register.form.button')" data-cy="submit">
        Register
      </button>
    </form>
  </div>
